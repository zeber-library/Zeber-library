...
function BookLayout() {
  ...
  const [type, setType] = useState('buybooks'); // You can set this dynamically based on your needs
      ...
      <div className="left-section">
        ...
        <div className="images">
          {book.coverUrls && book.coverUrls.map((url, index) => (
            <img key={index} src={url} alt={`Book Cover ${index + 1}`} id="book-cover" />
          ))}
        </div>
      </div>
      ...
      <div className="right-section">
        ...
        <div className="icons">
          ...
          <div className="icon">
            ...
            <div>{type === 'buybooks' ? '7 Days Replacement' : 'Return within 7 Days'}</div>
          </div>
          <div className="icon">
            <i className="fa-solid fa-truck"></i>
            <div>{type === 'buybooks' ? 'Amazon Delivered' : 'Local Delivery'}</div>
          </div>
          <div className="icon">
            <i className="fa-brands fa-amazon-pay"></i>
            <div>{type === 'buybooks' ? 'Pay on Delivery' : 'Payment Options'}</div>
          </div>
          <div className="icon">

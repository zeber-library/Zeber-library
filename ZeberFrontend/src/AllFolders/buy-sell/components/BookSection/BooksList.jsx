...
const BooksList = ({ apiType }) => {
  ...
  const settings = {
    ...
    responsive: [
      {
        breakpoint: 1440, // For larger screens like desktops or large laptops
        settings: {
          slidesToShow: 4,
          slidesToScroll: 1,
          infinite: true,
      ...
      {
        breakpoint: 1200, // For typical laptop screens
        settings: {
          slidesToShow: 3,
          slidesToScroll: 1,
          infinite: true,
      ...
      {
        breakpoint: 1024, // For tablets or smaller desktops
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
          infinite: true,
      ...
      {
        breakpoint: 320, // For very small devices like old phones or mini devices
        settings: {
          slidesToShow: 1,
          slidesToScroll: 1,
        },
    ...
    <Slider {...settings}>
      {Array.isArray(booksData) && booksData.length > 0 ? (
        booksData.map((book) => (
          <div key={book._id}>
            <Link
              to={`/${apiType === 'buybooks' ? 'buy' : 'sell'}/book/${book._id}`} 
              style={{ textDecoration: 'none' }}
            >
              <BookItem book={book} />
            </Link>
